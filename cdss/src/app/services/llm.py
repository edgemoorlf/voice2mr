...
class LLMService:
    def __init__(self):
        self.primary_client = OpenAI(base_url=LLM_API_URL, api_key="not used")
        self.fallback_client = OpenAI(base_url=FALLBACK_LLM_API_URL, api_key="not used")
    ...
    def _create_chat_completion(self, client: OpenAI, model: str, messages: List[Dict], response_format: Optional[Dict] = None):
        ...
        try:
            ...
            if "localhost" in base_url_str or "127.0.0.1" in base_url_str:
                # Ollama format - add JSON instruction to system message if needed
                ...
                if response_format:
                    # Add JSON instruction to messages for OpenAI as well
